...
## 2. Kernel eBPF Sensor (`v2/ebpf-c/src/sensor.c`)
...
Kernel and userspace share a standardized C ABI struct layout defined in `v2/ebpf-c/include/telemetry_events.h` and mirrored in Rust with `#[repr(C)]` in `v2/spectre-agent/src/telemetry.rs`:
...
## 4. Sigma AST Rule Engine (`v2/spectre-rules`)
...
## 5. Generational Process Graph (`v2/spectre-graph`)
...
Before evaluating rules in `v2/spectre-agent/src/enrichment.rs`, the agent enriches the event map:
...
## 7. Active Mitigation Subsystem (`v2/spectre-agent/src/mitigation.rs`)
...
|---|---|---|
| **Static Release Binary Size** | **4.8 MB** | `ls -lh v2/target/release/spectre-agent` (stripped release profile) |
...
| **Test Suite Execution** | **0.60s (all 15 tests pass)** | `cargo test --workspace --manifest-path=v2/Cargo.toml` |
...
|---|---|---|---|
| **eBPF Sensor** | Tracepoint Probes | `v2/ebpf-c/src/sensor.c` | `tracepoint_syscalls_sys_enter_execve` |
| **eBPF Sensor** | Telemetry Protocol | `v2/ebpf-c/include/telemetry_events.h` | `struct telemetry_header`, `struct telemetry_exec_event` |
| **Agent Core** | Entrypoint & GC Task | `v2/spectre-agent/src/main.rs` | `main`, background GC worker task |
| **Agent Core** | Deserialization | `v2/spectre-agent/src/telemetry.rs` | `TelemetryHeader`, `TelemetryExecEvent`, `TelemetryForkEvent` |
| **Agent Core** | Graph Enrichment | `v2/spectre-agent/src/enrichment.rs` | `enrich_event_from_graph` |
| **Agent Core** | Active Mitigation | `v2/spectre-agent/src/mitigation.rs` | `MitigationController`, `terminate_process_tree`, `pidfd_send_signal` |
| **Agent Core** | Pipeline Telemetry | `v2/spectre-agent/src/pipeline.rs` | `PipelineStats`, `start_metrics_reporter` |
| **Rule Engine** | Pratt Parser | `v2/spectre-rules/src/parser.rs` | `Parser::parse_expression`, `AstNode` |
| **Rule Engine** | Spec Evaluator | `v2/spectre-rules/src/evaluator.rs` | `SigmaEngine::evaluate`, value list OR/AND logic, CIDR matcher |
| **Graph Engine** | Generational Graph | `v2/spectre-graph/src/lib.rs` | `ProcessGraph`, `ProcessKey`, `evict_expired`, `get_ancestors` |
